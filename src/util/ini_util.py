...
class IniUtil:
    ...
    def get_ini_app_param(key: str):
        if (IniUtil.APP_MINI_MASK_CHECKED_KEY == key
                or IniUtil.APP_MINI_BREATHING_LIGHT_CHECKED_KEY == key) :
            return IniUtil.get_config_param(IniUtil.SETTINGS_KEY, key, fallback=True, as_type=bool)

        elif (IniUtil.APP_MINI_CHECKED_KEY == key
              or IniUtil.APP_TRAY_MENU_CHECKED_KEY == key
              or IniUtil.APP_FLASK_CHECKED_KEY == key
              or IniUtil.APP_ICON_FONT_BOLD_CHECKED_KEY == key):
